...
class ContainerRuntime:
    ...
    def run(
        ...
        # Add rootless options for podman
        ...
        for i, ro_path in enumerate(ro_mounts):
            cmd.extend(["-v", f"{ro_path}:/mnt/ro{i}:ro,Z"])
    ...
    def _add_credential_mounts(self, cmd: list[str], config: Config) -> None:
        ...
        if creds.github:
            ...
            if gh_config.exists():
                cmd.extend(["-v", f"{gh_config}:{host_home}/.config/gh:ro,Z"])

        if creds.azure:
            azure_dir = host_home / ".azure"
            if azure_dir.exists():
                cmd.extend(["-v", f"{azure_dir}:{host_home}/.azure:ro,Z"])

        if creds.aws:
            aws_dir = host_home / ".aws"
            if aws_dir.exists():
                cmd.extend(["-v", f"{aws_dir}:{host_home}/.aws:ro,Z"])

        if creds.gcloud:
            gcloud_dir = host_home / ".config" / "gcloud"
            if gcloud_dir.exists():
                cmd.extend(["-v", f"{gcloud_dir}:{host_home}/.config/gcloud:ro,Z"])
    ...
    def _add_claude_mounts(self, cmd: list[str], config: Config) -> None:
        ...
        if claude_config.global_claude_md and claude_config.global_claude_md.exists():
            cmd.extend([
                "-v", f"{claude_config.global_claude_md}:{host_home}/.claude/CLAUDE.md:ro,Z"
            ])

        if claude_config.plugins_dir and claude_config.plugins_dir.exists():
            cmd.extend([
                "-v", f"{claude_config.plugins_dir}:{host_home}/.claude/plugins:ro,Z"
            ])
